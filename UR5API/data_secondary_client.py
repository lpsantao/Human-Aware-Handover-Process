...
global hosts, mat
hosts = "192.168.0.103"   #"192.168.147.128"    #
mat = np.matrix
...
alpha = np.array([pi / 2, 0, 0, pi / 2, -pi / 2, 0])  # unit: radia
d = np.array([d1, d2, d3, d4, d5, d6])  # unit: mm
aa = np.array([a1, a2, a3, a4, a5, a6])  # unit: mm
...
class CartesianInfo:
    ...
    __slots__ = ['x', 'y', 'z', 'rx', 'ry', 'rz', 'tcp_offset_x', 'tcp_offset_y', 'tcp_offset_z', 'tcp_offset_rx',
                 'tcp_offset_ry', 'tcp_offset_rz']

    def unpack(self, buf):
        # parses Cartesian Info data
        ci = CartesianInfo()
        (_, _,
         ci.x, ci.y, ci.z, ci.rx, ci.ry, ci.rz, ci.tcp_offset_x, ci.tcp_offset_y, ci.tcp_offset_z, ci.tcp_offset_rx,
         ci.tcp_offset_ry, ci.tcp_offset_rz) = struct.unpack_from("!IB12d", buf)
        return ci
...
class AdditionalInfo:
    ...
    def unpack(self, buf):
        ...
        ai = AdditionalInfo()
        (_, _, ai.freedrive_button_pressed, ai.freedrive_button_enabled, ai.io_enabled_freedrive) = \
...
class send_UR():
    ...
    def HTM(self, ind, theta):
        ...
        Rot_z[0, 0] = Rot_z[1, 1] = cos(theta[ind])
        Rot_z[0, 1] = -sin(theta[ind])
        Rot_z[1, 0] = sin(theta[ind])
        ...
        Rot_x[1, 1] = Rot_x[2, 2] = cos(alpha[ind])
        Rot_x[1, 2] = -sin(alpha[ind])
        Rot_x[2, 1] = sin(alpha[ind])

        A_i = Rot_z * Trans_z * Trans_x * Rot_x

        return A_i

    def fwd_kin(self, theta, i_unit='r'):
        """
        ...
        Returns:
            poses: [x,y,z] of all joints w.r.t. base joint (list of numpy.array)
            ex: [array([0., 0., 0.0891]), array([0.2737, 0.2510, 0.29564]),...]
            poses[0] - access x,y,z of joint 0; poses[0][0] access x of joint 0
        """
        ...
        for i in range(6):
            ...
            poses.append(np.squeeze(np.asarray(((np.delete(T_06, 3, 0))[:, 3]).T)))
    ...
    def send_move(self, comm, pose, acc=0.2, vel=0.7, wait=True, threshold=None):
        """
